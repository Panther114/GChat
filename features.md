...
## Desktop App (Windows thin WebView2; macOS Tauri fallback)
- [x] Windows thin native shell (`src-desktop-win`) — wry/tao WebView2, tray, IPC, updater
- [x] Hosted UI bridge (`src-desktop-win/src/bridge.js`) — `window.electronAPI` surface
- [x] Tray-hide SPA suspend (tiny HTML) for lower idle RAM; restore reloads hosted app
- [x] Memory-oriented WebView2 flags (capped V8 heap, unused features off)
- [x] System tray icon with left-click restore/focus and right-click menu
- [x] Hide-to-tray on window close
- [x] Single-instance lock
- [x] Native OS notifications and focus-group routing
- [x] Auto-launch at system startup (registry)
- [x] GitHub Releases update check + install
- [x] Settings → Updates in-app check-for-updates UI
- [x] Open external links in the default browser
- [x] Windows NSIS thin installer (~1 MiB)
- [x] macOS Tauri/WKWebView fallback (`src-tauri/`, `build:mac`)
