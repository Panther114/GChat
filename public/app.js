(() => {
  ...
  async function loadGroupKeyVaultEntries() {
    for (const group of groups) {
      let entry = await keyVault.get(group.id);
  ...
  var GROK_CONTEXT_MESSAGE_LIMIT = 40;
  var GROK_CONTEXT_TOTAL_CHARS = 24e3;
  var AI_ASSISTANT_USER_ID = "__gchat_ai_grok__";
  var AI_ASSISTANT_NAME = "AI";
  var AI_ASSISTANT_COLOR = "#8d7bff";
  var AI_ASSISTANT_PROFILE_PICTURE = "/grok.webp";
  ...
  var DEFAULT_AI_MODEL = "grok-4-1-fast-non-reasoning";
  var AI_MODE_LABELS = {
    fast: "Context-less",
    thinking: "Context"
  };
  var DEFAULT_AI_MODE = "fast";
  var AI_TONE_LABELS = {
    casual: "Casual",
  ...
  var grokRequestInFlight = false;
  var grokResponseDraft = "";
  var grokResponseModel = "";
  var grokResponseMeta = null;
  var grokRequestSource = "panel";
  var grokRequestHashtag = null;
  var aiUsageSummary = null;
  var userManagementSummary = null;
  var whisperPickerMode = null;
  var pendingWhisperCommandStart = null;
  var desktopPointerEffectFrame = 0;
  var desktopPointerEffectBound = false;
  var desktopPointerEffectState = {
  ...
  var MSG_NO_KEY = "[Encryption unavailable]";
  var MSG_DECRYPT_FAIL = "[Unable to decrypt]";
  ...
  function applyActiveTagFilterToRenderedMessages() {
    ...
    for (const child of rows) {
      ...
      if (prevVisible && prevVisible.dataset.senderId && prevVisible.dataset.senderId === child.dataset.senderId && (prevVisible.dataset.hashtag || DEFAULT_TAG_TOPIC) === (child.dataset.hashtag || DEFAULT_TAG_TOPIC) && prevCreatedAt && createdAt) {
  ...
  async function getMessagePreviewText(msg, groupId = msg.groupId) {
    ...
    const aiMentionPrefix = msg.aiMention ? `${buildAiMentionLabel(msg.aiMeta)} ` : "";
    const prefix = getMessageHashtagPrefix(msg);
    const plaintext = Number(msg.encryptionVersion) === 2 ? await decryptV2Message(msg, key, groupId).catch(() => null) : await decryptMessage(msg.encryptedContent, msg.iv, key, groupId);
  ...
  async function renderMsgContent(msg, textEl, bubble, groupId = currentGroupId) {
    ...
    const plaintext = Number(msg.encryptionVersion) === 2 ? msg._decryptedText ?? await decryptV2Message(msg, key, groupId).catch(() => null) : await decryptMessage(msg.encryptedContent, msg.iv, key, groupId);
    if (plaintext === null) {
    ...
    socket.on("message_edited", async (updated) => {
      ...
      if (row) {
        ...
        if (bubble && textEl) {
          ...
          const key = currentGroupId ? getGroupKey(currentGroupId) : null;
          if (key) {
            const pt = Number(updated.encryptionVersion) === 2 ? await decryptV2Message(updated, key, currentGroupId).catch(() => null) : await decryptMessage(encryptedContent, iv, key, currentGroupId);
  ...
  function updateAskAiSubmitButton() {
    ...
    const hasSelections = !!AI_MODEL_OPTIONS[getSelectedAiModel()] && !!AI_MODE_LABELS[getSelectedAiMode()] && !!AI_TONE_LABELS[getSelectedAiTone()];
  ...
  async function buildGrokContextMessages(groupId, options = {}) {
    ...
    const sourceMessages = (snapshot || []).filter((msg) => !normalizedTag || getMessageHashtagKey(msg) === normalizedTag).slice(-GROK_CONTEXT_MESSAGE_LIMIT);
  ...
  async function requestAiResponse(groupId, options = {}) {
    ...
    const contextMessages = mode === "thinking" ? await buildGrokContextMessages(groupId, {
    ...
    if (!options.skipBusyUi) {
      ...
      setGrokBusy(true, mode === "thinking" ? contextMessages.length ? `Asking ${modelLabel} in ${modeLabel} mode\u2026` : `Asking ${modelLabel} in ${modeLabel} mode without chat context\u2026` : `Asking ${modelLabel} in ${modeLabel} mode\u2026`);
    }
  ...
  function setupEventListeners() {
    ...
    $("grok-submit-btn").addEventListener("click", () => {
...
${grokResponseDraft}` : grokResponseDraft;
      autoResizeTextarea(input);
      closeGrokModal();
