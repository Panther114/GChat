...
test('product version is 1.4.0 across thin Windows shell and macOS fallback', () => {
  assert.equal(packageJson.version, '1.4.0');
  assert.equal(tauriConfig.version, packageJson.version);
  assert.match(cargoWin, /version = "1.4.0"/);
  assert.match(cargoMac, /^version = "1.4.0"$/m);
});
...
test('macOS fallback Tauri stack remains documented and buildable', () => {
  ...
  assert.match(installDocs, /thin|WebView2|1.4.0/i);
  assert.match(packageJson.scripts['build:mac'], /tauri build/);
});
