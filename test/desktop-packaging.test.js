...
test('product version is 1.3.14 across thin Windows shell and macOS fallback', () => {
  assert.equal(packageJson.version, '1.3.14');
  assert.equal(tauriConfig.version, packageJson.version);
  assert.match(cargoWin, /version = "1.3.14"/);
  assert.match(cargoMac, /^version = "1.3.14"$/m);
});
...
test('macOS fallback Tauri stack remains documented and buildable', () => {
  ...
  assert.match(installDocs, /thin|WebView2|1.3.14/i);
  assert.match(packageJson.scripts['build:mac'], /tauri build/);
});
