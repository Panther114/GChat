...
const AI_CHANNEL_HISTORY_DEFAULT_LIMIT = 20;
...
function clearLocalFullHistoryProjection(groupId, { showNotice = false } = {}) {
  ...
  cache.knownChannels = new Set([DEFAULT_TAG_TOPIC]);
...
// v1.3.12: remove a single message from the durable history store when it is
...
// v1.3.12: bounded-parallel async map — decrypting hundreds of messages
// sequentially was the main reason chat history took seconds to appear.
...
async function applySyncEvents(groupId, events, cursor, { render = true } = {}) {
  ...
  const clearedChannelKeys = new Set();
  ...
  if (allChannelsAffected) {
    ...
    const hasMessageAfterClear = ordered.some((event) => event.type === 'message.created'
      && Number(event.seq) > lastFullHistoryClearSeq);
...
// v1.3.12: the last rendered message of the active channel (for append-only
// merges) — finds the final .msg-row in the transcript.
...
async function renderActiveChannelStream({ restoreScroll = true } = {}) {
  ...
  if (memo.rows.length > 0 && memo.lastMsgId === lastWindowId) {
    ...
    const valid = memo.rows.filter((row) => (
      !!row?.dataset?.msgId && cacheIds.has(String(row.dataset.msgId))
    ));
...
function selectTagChannel(topic, { focusComposer = true } = {}) {
  ...
  const renderPromise = hasCachedChannel
    ? renderActiveChannelStream().then(() => {
      ...
      return loadMessages(switchGroupId, undefined, { skipEmptyRender: true });
    })
...
const groupUnreadReconcileVersions = new Map();
...
function resetUnreadStateAfterFullHistoryClear(groupId) {
  ...
  channelUnreadTopicByTagIndex = new Map();
  updateFirstUnreadButton();
}
...
async function selectGroup(groupId) {
  ...
  // v1.3.9/1.3.12: hydrate durable IndexedDB history before first render so
  // the transcript is never blank while the server window loads — and ALWAYS
  // merge the recent window (not only when the cache is empty), so cached
  // history appears instantly instead of waiting on scroll-up fetches.
  const history = await readHistoryMessages(normalizedGroupId);
  if (history && history.length
    && (!cache.messages || history.length > cache.messages.length)) {
    const window = history.slice(-HISTORY_RENDER_WINDOW);
...
async function loadMessages(groupId, before, options = {}) {
  ...
  const cachedTopicHasMessages = Array.isArray(cacheAtStart.messages)
    ? cacheAtStart.messages.some((message) => resolveMessageTagTopic(message) === capturedTopic)
    : null;
  ...
  try {
    ...
    const msgs = filterMessagesVisibleToCurrentUser(rawMsgs);
    const skipCachedEmptyRender = skipEmptyRender
      && cachedTopicHasMessages === false
      && msgs.length === 0;
    if (!before) {
      const cache = ensureGroupCacheEntry(groupId);
...
const isReadByMe = isOwn || isMessageReadByCursor(msg, groupId, resolveMessageTagTopic(msg));
  ...
  // v1.3.12: keep decrypted text cached — re-renders (channel switches,
  // merges, reloads) skip the AES-GCM round-trip entirely.
  ...
  // Timestamp + delivery ticks (edited badge sits inline after the text)
  ...
  // v1.3.12: the edited marker belongs right behind the actual message text,
  // not at the far right with the delivery ticks.
...
function updateFirstUnreadButton() {
  ...
  if (first) {
    ...
    if (row && area) {
      const areaRect = area.getBoundingClientRect();
      // Show only when the first unread row has been scrolled past (above the
      // viewport) — clicking then scrolls smoothly up to it.
...
function jumpToFirstUnread() {
  ...
  // The unread message may live in older history — fetch it first, then jump.
  const area = messagesArea();
...
function initSocket() {
  ...
  socket.on('message_deleted', ({ messageId }) => {
    ...
    for (const [groupId, cache] of groupDataCache.entries()) {
      ...
      // v1.3.12: never let a deleted message resurrect from the durable store.
  ...
  socket.on('message_edited', async (updated) => {
    ...
    if (row) {
      ...
      if (bubble && textEl) {
        ...
        // v1.3.12: the "(edited)" marker sits inline right behind the message
        // text (not in the right-pinned tick meta).
        if (!bubble.querySelector('.msg-edited-badge')) {
          const badge = document.createElement('span');
          badge.className = 'msg-edited-badge';
          badge.textContent = ' (edited)';
          const textEl = bubble.querySelector('.msg-text');
          if (textEl && textEl.parentNode) textEl.after(badge);
...
let searchHistoryExhausted = false;
let searchReturnAnchor = null;
const SEARCH_PAGE_LIMIT = 100;
const SEARCH_AUTO_PAGE_BATCH = 5;
...
async function searchMessages(term, { pageBatch = SEARCH_AUTO_PAGE_BATCH } = {}) {
  ...
  try {
    ...
    const candidates = (ensureGroupCacheEntry(groupId).messages || [])
      .filter((msg) => resolveMessageTagTopic(msg) === channel)
      .filter((msg) => !isMessageHiddenForCurrentUser(msg));
    ...
    syncSearchHistoryStatus(
      searchHistoryExhausted
        ? `Searched all available history (${fetchedRows} older messages loaded).`
        : `Loaded ${fetchedRows} older messages in ${requests} bounded request${requests === 1 ? '' : 's'}.`
    );
...
// agent's history tools execute HERE against the local decrypted store. The
// server merely relays the tool calls; every read is bounded so one agent
// request can never scan or decrypt more than a fixed window.

function isAiReadableMessage(msg) {
  return !!(
    msg
    && (msg.type === 'text' || msg.type == null)
    && !(Array.isArray(msg.whisperTo) && msg.whisperTo.length)
    && !isDisappearingMessage(msg)
  );
}
...
function truncateAiToolResult(result) {
  ...
  // Keep the newest entries first — history is most relevant close to the ask.
  const kept = [];
  let used = 2;
...
async function collectAiToolMessages(groupId) {
  ...
  let historyMessages = [];
  try {
    // Durable IndexedDB history holds paginated older messages.
...
async function executeAiToolGetChannelHistory(input, options) {
  ...
  const limit = Number.isFinite(rawLimit)
    ? Math.max(1, Math.min(AI_CHANNEL_HISTORY_MAX_LIMIT, Math.round(rawLimit)))
    : AI_CHANNEL_HISTORY_DEFAULT_LIMIT;
  ...
  const candidates = combined
    .filter((m) => isAiReadableMessage(m) && resolveMessageTagTopic(m) === channel)
    .sort((a, b) => {
      const timeDiff = String(a.createdAt || '').localeCompare(String(b.createdAt || ''));
      return timeDiff !== 0 ? timeDiff : String(a.id).localeCompare(String(b.id));
    });
  ...
  if (before) {
    ...
    if (beforeIndex >= 0) window = candidates.slice(0, beforeIndex);
  }
...
async function executeAiToolGetChannelList(options) {
  ...
  for (const m of combined) {
    ...
    knownTopics.add(topic);
    const entry = stats.get(topic) || { messageCount: 0, lastMessageAt: '', senders: new Set() };
...
// v1.4: run one full agent request — round-trips with the server until the
// model either answers directly or stops requesting tools. The transcript is
...
async function sendAiPromptMessage(parsedMessage, tone = getSelectedAiTone()) {
  ...
  try {
    ...
    // the full v2 envelope (id/encryptedMetadata/metadataIv/replyToId/tagIndex).
    const messageId = crypto.randomUUID();
    const messageIdentity = {
      id: messageId,
      groupId,
    ...
    const tagIndex = hashtag && hashtag !== DEFAULT_TAG_TOPIC
      ? await GChatCryptoV2.blindIndex(hashtag, key, groupId, 'tag-index')
      : null;
...
// v1.3.12: serve scroll-up pagination from the durable IndexedDB history when
// the requested older messages are already cached — instant, no network.
async function prependHistoryMessagesOlderThan(cursorId) {
  ...
  const older = history
    ...
    .filter((m) => !isMessageHiddenForCurrentUser(m));
  ...
  if (!channelMsgs.length) {
    // Only non-active-channel messages were cached — merge them but don't
    // prepend, so the network path can still advance the cursor.
