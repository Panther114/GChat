...
- **C1 — socket handlers can no longer crash the server**: `send_message`, `send_whisper`, `send_ai_message`, the disappearing-timer handlers, and the upload-progress relays now normalize client ids (`String().slice(0,64)`) before any `better-sqlite3` binding and wrap DB work in try/catch. A malformed packet (e.g. an object `groupId`) previously threw inside a Socket.IO listener — an uncaughtException that killed the whole process for everyone.
- **C2 — `channel_read_cursors` no longer grows unboundedly**: `#main` cursors are stored with `tag_index NULL`, and SQLite treats NULLs as distinct in unique constraints, so every `mark_channel_read` inserted a brand-new row. A one-shot dedupe (keep the newest per group+user) plus a partial unique index make future upserts conflict correctly.
- **H1 — read cursors are monotonic**: client and server now refuse to move a read cursor backwards. Stale viewport replays (after a reload) used to regress the cursor and make already-read messages look unread again (badge, chips, `.unseen` bar).
- **H2 — edits can no longer revert**: the edit handler now invalidates the cached plaintext (and decrypted channel) and persists the edited envelope to the durable IndexedDB store; the optimistic-echo reconcile also refuses to overwrite a message that was already edited. Previously a rebuild or offline reload re-rendered the pre-edit text.
- **H3 — since-sync can no longer stall behind an invisible page**: the persisted sync cursor now advances from the RAW fetched page, so a full page of whispers the viewer can't access no longer blocks delivery of every visible message behind it.
- **H4 — `?limit=` is clamped to 1–100**: a negative limit previously made SQLite return the entire group history in one response.
- **H6 — login/registration regenerate the session**: the pre-auth session id (and its CSRF token) can never be re-used post-login; the token rotates with the new session.
- **H7 — whisper recipient lists are capped** at the member limit; an unbounded list was an event-loop DoS (a synchronous membership query per recipient).
- **H8 — desktop shells hardened**: the WebView2/Electron bridges now only install and process IPC from trusted pages (the official hosted app or the offline recovery page). Arbitrary `data:`/`about:`/`file:` navigation is blocked, `window.open`/external links are restricted to https, and the Electron clipboard handler is size-bounded. A single XSS in the hosted app can no longer escalate to shell commands (install-update, clear-cache-and-restart, clipboard).
- **Docs**: clarified that "Furina" is the operator's personal app-owner account — it exists, is actively used, and will not be deleted (see README).
